...
def test_stactrl_pf_import(): ##todo
...
def test_volt_ctrl_droop_new():
    ...
    DroopControl(net, q_droop_mvar=40, controller_idx=bsc.index, control_modus='V_ctrl', input_element_q_meas='res_trafo',
...
def test_qctrl_droop_new():
    ...
    DroopControl(net, q_droop_mvar=40, bus_idx=1,
                 vm_set_pu=1, vm_set_ub=1.005, vm_set_lb=0.995,
                 controller_idx=bsc.index, control_modus='Q_ctrl')
    runpp(net, run_control=False)
    assert(abs(net.res_line.loc[0, "q_to_mvar"] - (-7.094325e-13)) < tol)
    runpp(net, run_control=True)
    assert(all(net.controller.object[i].converged == True for i in net.controller.index))
...
def test_pf_control_droop_q():
    ...
    bsc = BinarySearchControl(net, ctrl_in_service=True,
                                         ...
                                         output_element_in_service=True, output_values_distribution='max_Q',
                                         input_element="res_line", damping_factor=0.9, input_variable="q_to_mvar",
                                         input_element_index=0, set_point=1,control_modus='PF_ctrl', tol=1e-6)
    DroopControl(net, bus_idx=1, pf_overexcited= 0.5, pf_underexcited= 0.9,
                 vm_set_pu=1, vm_set_ub=3, vm_set_lb=1,
                 controller_idx=bsc.index, control_modus='PF_ctrl_P')
    runpp(net, run_control=False)
...
def test_pf_control_droop_v():
    ...
    bsc = BinarySearchControl(net, ctrl_in_service=True,
                                         ...
                                         input_element="res_line", damping_factor=0.9, input_variable="q_to_mvar",
                                         input_element_index=0,control_modus='PF_ctrl', tol=1e-6, set_point=0.5)
    DroopControl(net, bus_idx=1, pf_overexcited=-0.3, pf_underexcited=0.7,
                 vm_set_pu=1, vm_set_ub=0.6, vm_set_lb=2.2,
                 controller_idx=bsc.index, control_modus='PF_ctrl_V')
    runpp(net, run_control=False)
    assert(abs(np.arctan(net.res_line.loc[0, "q_to_mvar"] / net.res_line.loc[0, 'p_to_mw']) - 0) < tol)
...
def test_station_ctrl_pf_import_new():
    ...
    assert(net.controller.at[3, 'object'].control_modus == 'V_ctrl')  # test correct control_modus
    assert(net.controller.at[4, 'object'].control_modus == 'V_ctrl')  # test correct control_modus
    assert(net.controller.at[4, 'object'].controller_idx == 3)  # test droop controller linkage
    print("--------------------------------------")
    print("Scenario 3 - Constant V")
    ...
    assert(net.controller.at[1, 'object'].control_modus == 'Q_ctrl')  # test correct control_modus
    assert(net.controller.at[2, 'object'].control_modus == 'Q_ctrl')  # test correct control_modus
    assert(net.controller.at[2, 'object'].controller_idx == 1)  # test droop controller linkage
    print("------------------------------------- ")
    ...
    assert(net.controller.at[6, 'object'].control_modus == 'PF_ctrl')  # test correct control_modus
    print("------------------------------------- ")
    ...
    assert(abs(np.arctan(net.res_line.loc[27, "q_to_mvar"] / net.res_line.loc[27, 'p_to_mw']) - np.arccos(
        droop_set_point)) < tol)
    assert(net.controller.at[8, 'object'].control_modus == 'PF_ctrl')  # test correct control_modus
    assert(net.controller.at[9, 'object'].control_modus == 'PF_ctrl')  # test correct control_modus
    assert(net.controller.at[9, 'object'].p_cosphi == False) #correct control_modus
    ...
    assert(abs(np.arctan(net.res_line.loc[25, "q_to_mvar"] / net.res_line.loc[25, 'p_to_mw']) - np.arccos(
        droop_set_point)) < tol)
    assert(net.controller.at[10, 'object'].control_modus == 'PF_ctrl')  # test correct control_modus
    assert(net.controller.at[11, 'object'].control_modus == 'PF_ctrl')  # test correct control_modus
...
def test_station_ctrl_pf_import_distributions():#test comparability between PF and pp
    ...
    assert(net.controller.at[3, 'object'].control_modus == 'PF_ctrl')  # test correct control_modus
    assert(net.controller.at[4, 'object'].control_modus == 'tan(phi)_ctrl')  # test correct control_modus
